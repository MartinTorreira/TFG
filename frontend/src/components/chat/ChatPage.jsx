...
const ChatPage = ({ setSelectedConversationId, selectedConversationId }) => {
  ...
  return (
    <div className={`flex h-[600px] ${isAnimating ? "slide-up" : ""}`}>
      <div className="w-1/3 p-4 bg-gray-100 h-full overflow-y-auto">
        ...
        {Object.keys(conversations).map((conversationId) => {
          ...
          return (
            <div
              key={conversationId}
              className="flex flex-row cursor-pointer hover:bg-gray-200 p-2 mt-2 -ml-2 border-y"
      ...
      <div className="flex flex-col items-center justify-center w-3/4 bg-white overflow-hidden h-full">
        {selectedConversationId && (
          <div className="w-full p-4 bg-gray-200 flex items-center">
            <img
              src={
                userDetails[
                  selectedConversationId
          ...
          {selectedConversationId && conversations[selectedConversationId] ? (
            conversations[selectedConversationId].messages.map((msg, index) => (
              ...
              >
                <div>
                  <div
                    className={`p-3  ${msg.senderId === user.id ? "bg-accent-dark text-white rounded-l-lg rounded-br-xl" : "bg-gray-300 rounded-r-lg rounded-bl-xl"}`}
                  >
        ...
        <div className="bg-gray-100 p-2 px-0 mr-2 flex flex-row space-x-2 w-full">
          <input
            className="flex items-center h-10 w-full rounded px-3 text-sm focus:ring-0"
            type="text"
