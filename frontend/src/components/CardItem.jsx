...
export const CardItem = ({ path, favorite, cart }) => {
  return (
    <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 hover:scale-105 transition-all group">
      ...
      <a href="#">
        <img className="w-full object-cover rounded-t-lg group-hover:opacity-30" src={path} alt="product image" />
      </a>
      <div className="px-5 pb-5 mt-4">
        <a href="#">
          <h5 className="mt-2 font-bold text-gray-600 font-smemibold tracking-tight dark:text-white">Apple Watch Series 7 GPS, Aluminium Case, Starlight Sport</h5>
        </a>
        <div className="flex items-center justify-between gap-x-2 mt-4">
          <span className="text-2xl font-bold text-gray-900 dark:text-white">599€</span>

          <div className="flex flex-row gap-x-3">
            <button title={favorite ? "Quitar de favoritos" : "Añadir a favoritos"} className="border-2 border-black/20 p-2 rounded-lg">
              {favorite ? <FavoriteIconFilled size={24} /> : <FavoriteIcon size={24} />}
            </button>
            <button title={cart ? "Quitar del carro" : "Añadir al carro"} className="border-2 border-black/20 p-2 rounded-lg" >
